...
static int ov5640_af_init(struct ov5640_dev *ov5640)
{
	...
	if (ret < 0)
	{
		dev_err(&ov5640->i2c_client->dev, "Focus initialization: cant reset the MCU\n");
		return ret;
	}
	...
	{
		ret = ov5640_write_reg(ov5640, addr, AF_Firmware[sentCnt]);

		if (ret < 0)
		{
			dev_err(&ov5640->i2c_client->dev, "Focus initialization: failed writing reg: 0x%x\n", addr);
			return ret;
		}
	}
	...
	msleep(10);
	ov5640_write_reg(ov5640, 0x3000, 0x00); // start MCU

	ov5640_read_reg(ov5640, 0x3004, &state);
	state |= 0x01 <<5;
	state |= 0x01 <<6;
	ov5640_write_reg(ov5640, 0x3004, state); //set 0x3004 BIT6=1,BIT5=1

	ov5640_read_reg(ov5640, 0x3001, &state);
	state &= ~(0x01 <<6);
	ov5640_write_reg(ov5640, 0x3001, state); //set 0x3001 BIT6=0		

	ov5640_read_reg(ov5640, 0x3005, &state);
	state |= 0x01 <<6;
	ov5640_write_reg(ov5640, 0x3005, state); //set 0x3005 BIT6=1	

	// Wait for up tp 5000ms to converge
	for (remaining = 5000;;)
	{
		ret = ov5640_read_reg(ov5640, 0x3029, &state);

		if (ret < 0)
		{
			dev_err(&ov5640->i2c_client->dev, "Focus initialization: cant read status reg 0x3029\n");
			break;
		}
		...
		if (remaining <= 0)
		{
			dev_err(&ov5640->i2c_client->dev, "Focus initialization: timed out waiting to complete (3029: 0x%x\n", state);
			ret = -1;
			break;
		}
	}
...
static int ov5640_set_af_mode(struct ov5640_dev *ov5640, s32 focusmode, s32 param)
{
	int ret = 0;
	u8 state = 0x8F;
	int remaining ;
	...
	case V4L2_CID_FOCUS_AUTO:
		dev_warn(&ov5640->i2c_client->dev, "V4L2_CID_FOCUS_AUTO\n");
	...
	case V4L2_CID_AUTO_FOCUS_START:
		...
		dev_warn(&ov5640->i2c_client->dev, "V4L2_CID_AUTO_FOCUS_START\n");
		ov5640_write_reg(ov5640, 0x3022, 0x03);
		return(0);

	case V4L2_CID_3A_LOCK:
		dev_warn(&ov5640->i2c_client->dev, "V4L2_CID_3A_LOCK\n");
	...
	case V4L2_CID_AUTO_FOCUS_STOP:
		dev_warn(&ov5640->i2c_client->dev, "V4L2_CID_AUTO_FOCUS_STOP\n");
	...
	for (remaining = 3000;;)
	{
		ret = ov5640_read_reg(ov5640, 0x3023, &state);
		...
		if (state == 0x00)
		{
			dev_warn(&ov5640->i2c_client->dev, "Focus succeeded in %dms\n", remaining);
			break;
		}
		...
		if (remaining <= 0)
		{
			dev_warn(&ov5640->i2c_client->dev,"Focus failed: timed out\n");
			ret = -1;
			break;
		}
...
static int ov5640_set_af_window(struct ov5640_dev *ov5640, int param)
{
	...
	case V4L2_AUTO_FOCUS_RANGE_INFINITY:
		...
		ov5640_set_af_mode(ov5640, V4L2_CID_FOCUS_AUTO, 0);
		return(0);

	default:
		dev_err(&ov5640->i2c_client->dev, "Focus mode unknown: %x\n", param);
		return(-1);
	}
...
static int ov5640_setup_af(struct ov5640_dev *ov5640)
{
	...
	ret = ov5640_read_reg(ov5640, 0x8025, &state);
	if(ret < 0 || state == 0x00){
		dev_warn(&ov5640->i2c_client->dev, "This is probably the closing, turning off af\n");
		ov5640_set_af_mode(ov5640, V4L2_CID_FOCUS_AUTO, 0);
		return(0);
	}
	else if(state == 0x90) {
		dev_warn(&ov5640->i2c_client->dev, "AFirmware already written, skipping\n");
	} else {
		dev_warn(&ov5640->i2c_client->dev,"Autofocus initialization started, 0x8025 state is %x\n", state);
		ret = ov5640_af_init(ov5640);

		if (ret < 0) {
			dev_err(&ov5640->i2c_client->dev, "Autofocus initialization failed\n");
			return(ret);
		}
	}
	// Set to normal window
	ov5640_set_af_window(ov5640, V4L2_AUTO_FOCUS_RANGE_NORMAL);
	ov5640_set_af_mode(ov5640, V4L2_CID_FOCUS_AUTO, 1);
	dev_warn(&ov5640->i2c_client->dev, "Autofocus initialization succeeded\n");
	ov5640->focus_state = V4L2_AUTO_FOCUS_STATUS_IDLE;
	return(0);
}


static int ov5640_get_af_status(struct ov5640_dev *ov5640, int *focus)
{
	int ret = 0;
	u8 state;
	...
	ret = ov5640_read_reg(ov5640, 0x3029, &state);
	if (ret < 0) {
		dev_err(&ov5640->i2c_client->dev, "Autofocus status failed\n");
		return(ret);
	}
...
static int ov5640_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_AUTO_FOCUS_STATUS:
		val = ov5640_get_af_status(sensor, &ctrl->val);
		if (val < 0) {
			dev_err(&sensor->i2c_client->dev, "Get ctrl: V4L2_CID_AUTO_FOCUS_STATUS failed\n");
		}
		break;
	}
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VFLIP:
		...
		break;
	case V4L2_CID_FOCUS_AUTO:
		dev_warn(&sensor->i2c_client->dev, "Set ctrl: V4L2_CID_FOCUS_AUTO, param=%x\n", ctrl->val);
		ret = ov5640_set_af_mode(sensor, V4L2_CID_FOCUS_AUTO, ctrl->val);
		break;
	case V4L2_CID_AUTO_FOCUS_START:  // single shot auto focus start
		dev_warn(&sensor->i2c_client->dev, "Set ctrl: V4L2_CID_AUTO_FOCUS_START, param=%x\n", ctrl->val);
		ret = ov5640_set_af_mode(sensor, V4L2_CID_AUTO_FOCUS_START, ctrl->val);
		break;
	case V4L2_CID_3A_LOCK:
		dev_warn(&sensor->i2c_client->dev, "Set ctrl: V4L2_CID_3A_LOCK, param=%x\n", ctrl->val);
		ret = ov5640_set_af_mode(sensor, V4L2_CID_3A_LOCK, ctrl->val);
		break;
	case V4L2_CID_AUTO_FOCUS_STOP:  // single shot auto focus stop
		dev_warn(&sensor->i2c_client->dev, "Set ctrl: V4L2_CID_AUTO_FOCUS_STOP, param=%x\n", ctrl->val);
		ret = ov5640_set_af_mode(sensor, V4L2_CID_AUTO_FOCUS_STOP, ctrl->val);
		break;
	case V4L2_CID_AUTO_FOCUS_RANGE:
		dev_warn(&sensor->i2c_client->dev, "Set ctrl: V4L2_CID_AUTO_FOCUS_RANGE, param=%x\n", ctrl->val);
		ret = ov5640_set_af_window(sensor, ctrl->val);
		break;
	default:
		ret = -EINVAL;
		break;
